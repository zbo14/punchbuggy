...
class Session {
  ...
  setRemoteInfo (rinfo) {
    ...
    const portDelta = Buffer.alloc(2)

    console.log(this.portDelta)

    port.writeUint16BE(this.port)
    portDelta.writeUint16BE(this.portDelta)

    this.contact = Buffer.concat([addr, port, portDelta])
  }
  ...
  async handleMessage (msg, rinfo) {
    switch (msg.type) {
      ...
      case util.MESSAGES.PORT_REQUEST: {
        await this.handlePortRequest(msg)
        return
      }
  ...
  handleConnectRequest (msg, rinfo) {
    const unexpected = (
      (this.isTCP && this.state !== SESSION_STATES.IDENTIFIED) ||
      (this.isUDP && ![
    ...
    this.sendConnectResponse(session, true, msg)
  }
  ...
  async handleIdRequest (msg) {
    ...
    this.portDelta = msg.body.slice(3).readUint16BE()
    this.proto = proto
  ...
  async handlePortRequest (msg) {
    const body = Buffer.alloc(2)
    body.writeUint16BE(this.sock.remotePort)

    this.send(util.MESSAGES.PORT_RESPONSE, msg.nonce, body)
    this.end()
  }
  ...
  sendConnectResponse (session, initiate = false, msg = {}) {
    const body = Buffer.from([...session.contact, +initiate])
    this.send(util.MESSAGES.CONNECT_RESPONSE, msg.nonce || this.nonce, body)
